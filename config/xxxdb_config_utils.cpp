...
std::unique_ptr<XXXDBConfig> GenerateXXXDBConfig(
  ...
  *(*self_info).mutable_certificate_info() = cert_info;
  ...
  return std::make_unique<XXXDBConfig>(replicas, self_info.value(), private_key,
                                       cert_info);
}

XXXDBConfig GenerateXXXDBConfig(const std::string &config_file) {
  std::vector<ReplicaInfo> replicas = ReadConfig(config_file);
  return XXXDBConfig(replicas, ReplicaInfo());
}
