...
void TransactionAccessor::TransactionFetching() {
  ...
  while (!stop_) {
	  ...
	  LOG(ERROR)<<"add seq :"<<max_received_seq_<<" cur:"<<cur_seq<<" spend:"<<get_current_time() - current_time1<<" "<<(*ret).size()<<" new:"<<get_sys_clock()<<" current time:"<<get_current_time();
...
TransactionAccessor::ConsumeTransactions(uint64_t seq) {
  ...
  batch_transactions->set_max_seq(seq + config_.BatchTransactionNum() + next_want - 1);
