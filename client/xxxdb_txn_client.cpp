...
XXXDBTxnClient::XXXDBTxnClient(const XXXDBConfig &config)
    : config_(config), replicas_(config.GetReplicaInfos()),
      recv_timeout_(1) /*1s*/ {}

std::unique_ptr<XXXDBClient>
XXXDBTxnClient::GetXXXDBClient(const std::string &ip, int port) {
...
XXXDBTxnClient::GetTxn(uint64_t min_seq, uint64_t max_seq) {
  QueryRequest request;
  request.set_min_seq(min_seq);
  request.set_max_seq(max_seq);

  std::vector<std::unique_ptr<XXXDBClient>> clients;
  ...
  for (const auto &replica : replicas_) {
    ...
    XXXDBClient *client_ptr = client.get();
