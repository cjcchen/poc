...
XXXDBReplicaClient::XXXDBReplicaClient(const std::vector<ReplicaInfo> &replicas,
...
XXXDBReplicaClient::~XXXDBReplicaClient() {
  is_running_ = false;
...
bool XXXDBReplicaClient::IsInPool(const ReplicaInfo &replica_info) {
...
bool XXXDBReplicaClient::IsRunning() const { return is_running_; }

void XXXDBReplicaClient::UpdateClientReplicas(
    const std::vector<ReplicaInfo> &replicas) {
  clients_ = replicas;
}

std::vector<ReplicaInfo> XXXDBReplicaClient::GetClientReplicas() {
  return clients_;
}

int XXXDBReplicaClient::SendHeartBeat(const HeartBeatInfo &hb_info) {
  int ret = 0;
  for (const auto &replica : replicas_) {
    XXXDBClient client(replica.ip(), replica.port());
...
void XXXDBReplicaClient::StartBroadcastInBackGround() {
  is_running_ = true;
...
int XXXDBReplicaClient::SendMessage(const google::protobuf::Message &message) {
  global_stats_->BroadCastMsg();
  if (is_use_long_conn_) {
    auto item = std::make_unique<QueueItem>();
    item->data = XXXDBClient::GetRawMessageString(message, verifier_);
...
int XXXDBReplicaClient::SendMessage(const google::protobuf::Message &message,
  ...
  if (is_use_long_conn_) {
    std::string data = XXXDBClient::GetRawMessageString(message, verifier_);
    BroadcastData broadcast_data;
...
int XXXDBReplicaClient::SendMessageFromPool(
...
int XXXDBReplicaClient::SendMessageInternal(
    const google::protobuf::Message &message,
    const std::vector<ReplicaInfo> &replicas) {
  int ret = 0;
  for (const auto &replica : replicas) {
...
AsyncReplicaClient *XXXDBReplicaClient::GetClientFromPool(const std::string &ip,
                                                          int port) {
...
XXXDBReplicaClient::GetClient(const std::string &ip, int port) {
  return std::make_unique<XXXDBClient>(ip, port);
}

void XXXDBReplicaClient::BroadCast(const google::protobuf::Message &message) {
...
void XXXDBReplicaClient::SendMessage(const google::protobuf::Message &message,
                                     int64_t node_id) {
