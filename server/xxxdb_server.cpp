...
XXXDBServer::XXXDBServer(const XXXDBConfig &config,
                         std::unique_ptr<XXXDBService> service)
  ...
  async_acceptor_ = std::make_unique<AsyncAcceptor>(
      ...
      std::bind(&XXXDBServer::AcceptorHandler, this, std::placeholders::_1,
                std::placeholders::_2));
  async_acceptor_->StartAccept();
  global_stats_ = Stats::GetGlobalStats();
}

XXXDBServer::~XXXDBServer() {}

void XXXDBServer::AcceptorHandler(const char *buffer, size_t data_len) {
  BroadcastData data;
...
void XXXDBServer::Run() {
  service_->Start();

  auto input_th = std::thread(&XXXDBServer::InputProcess, this);
...
void XXXDBServer::Process(std::unique_ptr<QueueItem> item) {
  auto client_socket =
  ...
  context->client = std::make_unique<XXXDBClient>(std::move(client_socket),
                                                  /*connected=*/true);
  if (request_info) {
...
bool XXXDBServer::IsRunning() { return service_->IsRunning(); }

void XXXDBServer::Stop() {
  socket_->Close();
  service_->Stop();
}

bool XXXDBServer::ServiceIsReady() const { return service_->IsReady(); }
