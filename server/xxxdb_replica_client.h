...
// XXXDBReplicaClient is used for replicas to broadcast messages.
class XXXDBReplicaClient {
public:
  XXXDBReplicaClient(const std::vector<ReplicaInfo> &replicas,
                     SignatureVerifier *verifier = nullptr,
                     bool is_use_long_conn = false, int epoll_num = 1);
  virtual ~XXXDBReplicaClient();
...
protected:
  virtual std::unique_ptr<XXXDBClient> GetClient(const std::string &ip,
                                                 int port);
